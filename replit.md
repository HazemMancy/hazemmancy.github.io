...
- **Two-Phase Separator (7-Tab Wizard)**: Project → Cases → Design → Gas Sizing → Holdup → Geometry → Results. GPSA Section 7 gas scrubber/inlet separator sizing. Service types: gas scrubber, inlet separator, slug catcher, test separator. Souders-Brown with GPSA K-factors. GPSA Fig 7-9 pressure correction and foam derating applied to effective K. Optional Stokes' law droplet settling verification with particle Reynolds number check (Re_p). Horizontal settling length verification. Slug catcher volume accommodation. Engineering flags include FOAM_K_DERATED, PRESSURE_K_CORRECTED, SETTLING_LENGTH_SHORT, STOKES_RE_EXCEEDED. Engine: `twoPhaseSeparator.ts`.
- **Flare KO Drum (7-Tab Wizard)**: Project → Scenarios → Design → Gas Sizing → Liquid Accumulation → Geometry → Results. API 521 knockout drum sizing for flare/vent systems. Conservative K-factors (0.02-0.06 gravity settling). API 521 minimum vessel diameter enforcement (1500 mm / 60"). Optional GPSA pressure correction (disabled by default for near-atmospheric service). Multi-scenario relief/blowdown cases with liquid accumulation from transient events (governing scenario, non-concurrent). Drain rate adequacy check. Engineering flags include MINIMUM_DIAMETER_APPLIED, PRESSURE_K_CORRECTED. Engine: `flareKODrum.ts`.
