...
- **Flare KO Drum Sizing (Fully Revised)**: A 7-tab wizard per API 521 Section 5.4.2. Key revisions: (1) `HoldupBasis.holdupTime` field removed — it was unused in the engine; liquid design volume is now explicitly based on governing-scenario carryover accumulation (Q_l × event duration) + rainout allowance − drain credit, with sump dead volume handled via sump zone height in geometry; (2) Liquid accumulation step labels fully rewritten to be explicit about the governing-scenario, non-concurrent basis; (3) `TRANSIENT_REVIEW` flag changed from always-on to only trigger when blowdown/depressuring scenarios are present; (4) `DYNAMIC_SIM_REQUIRED` flag now only triggers when ≥2 transient scenarios OR transient + high liquid accumulation — not for any multi-scenario case; (5) All flag labels rewritten with precise engineering language; (6) Recommendations for TRANSIENT_REVIEW and DYNAMIC_SIM_REQUIRED softened to "assess whether" / "may be warranted" rather than automatic; (7) `MINIMUM_DIAMETER_APPLIED` flag text now explicitly states "screening guideline, not an unconditional universal requirement"; (8) Gross vessel face velocity consistently labeled as such (gross cross-sectional area basis) — never "mist eliminator face velocity"; (9) Drain adequacy UI text now explicitly says "preliminary screening indicator only — not final drain system design"; (10) Standard gas basis (15°C, 1.01325 bar(a) ISO 13443) now shown inline in scenarios tab when standard flow basis is selected; (11) `flareKOScenarioSchema`, `flareKOAllowancesSchema`, `flareKOConfigSchema`, `flareKOHoldupSchema` Zod schemas added to validation.ts; `safeParse()` called at top of `handleCalculate` before invoking solver — validates all scenarios, config, and holdup; (12) Prominent screening banner added at the top of the page with 4 explicit limitations; (13) Page subtitle updated to "Flare KO Drum Preliminary Sizing / Screening Tool — API 521 Section 5.4.2"; (14) Export methodology fully rewritten with explicit per-topic disclosures (K correction, liquid volume basis, drain check, gross face velocity, minimum diameter, dynamic simulation); (15) Assumptions block fully rewritten — scope disclaimer first, explicit liquid approach, sump treatment, drain adequacy limitations, gross face velocity disclaimer, standard gas basis, minimum diameter guideline.
