...
- **Wizard-style UI**: Complex calculators (e.g., Restriction Orifice, Control Valve, Conventional Separator, Two-Phase Separator, Flare KO Drum, Heat Exchanger, PRD/Flare Relief, Compressor, Pump Sizing) utilize multi-tab wizard interfaces for guided input and calculation flow. All separator calculators and Heat Exchanger share a consistent step progress bar pattern: clickable step buttons with numbered circles (✓ for completed steps), hidden TabsList, `StepHeader` component at top of each tab content, and bottom navigation bar with Back/Step X of Y/Next buttons. Calculate button appears on the last input tab (holdup/liquid accumulation); Results tab shows Recalculate. Engineering flags banner displayed between progress bar and tab content when results exist.
- **Orientation Recommendation Engine**: Shared `orientationRecommendation.ts` module computes GVF-based vertical/horizontal recommendations per GPSA Section 7 and API 12J/521. Uses conservative minimum GVF across all operating cases. GVF > 0.85 → vertical (strong); GVF < 0.50 → horizontal (strong); 0.50–0.85 → either (moderate) with tie-breakers for 3-phase, foam, slugging, solids. Slug catcher always horizontal. Flare KO drums biased vertical per API 521. Recommendation panel with confidence badges shown on Design tab of all separator calculators.
- **Conventional Separator (7-Tab Wizard)**: Project → Cases → Design → Gas Sizing → Holdup → Geometry → Results. API 12J production separator sizing. Vertical/horizontal orientations with GVF-based orientation recommendation panel, 2-phase and 3-phase (gas-oil-water) configurations. Souders-Brown gas capacity sizing with API 12J K-value guidance. GPSA Fig 7-9 pressure-dependent K correction (Cp piecewise linear: 1.0 for P≤6.9 barg, decreasing above, min 0.5). Foam derating factor (0.5-0.7) applied when foam flagged. Effective K = K_base × Cp × F_foam with full CalcStep trace. Horizontal settling length check (t_settle vs t_gas). Stokes Re_p validation (warn if >1). API 12J seam-to-seam height buildup for vertical. Liquid holdup/surge design with 3-phase oil/water retention times. Engineering flags include FOAM_K_DERATED, PRESSURE_K_CORRECTED, SETTLING_LENGTH_SHORT, STOKES_RE_EXCEEDED. Engine: `conventionalSeparator.ts`.
- **Two-Phase Separator (7-Tab Wizard)**: Project → Cases → Design → Gas Sizing → Holdup → Geometry → Results. GPSA Section 7 gas scrubber/inlet separator sizing. Service types: gas scrubber, inlet separator, slug catcher, test separator. Supports 2-phase (gas/liquid) and 3-phase (gas/oil/water) modes — 3-phase available for inlet separator and test separator service types. GVF-based orientation recommendation panel. Souders-Brown with GPSA K-factors. GPSA Fig 7-9 pressure correction and foam derating applied to effective K. Optional Stokes' law droplet settling verification with particle Reynolds number check (Re_p). Horizontal settling length verification. Slug catcher volume accommodation. 3-phase adds oil/water properties per case (waterCut, oilDensity, waterDensity, oilViscosity, waterViscosity) and separate oil/water retention times in holdup. Engineering flags include FOAM_K_DERATED, PRESSURE_K_CORRECTED, SETTLING_LENGTH_SHORT, STOKES_RE_EXCEEDED, THREE_PHASE_PRELIM. Engine: `twoPhaseSeparator.ts`.
