...
- **Compressor Sizing (5-Tab Wizard)**: Project → Gas & Conditions → Sizing → Stages → Summary. Supports centrifugal (API 617) and reciprocating (API 618) compressors with polytropic/isentropic models per GPSA Section 13. Auto-staging based on compression ratio and discharge temperature limits. Mass flow via molar volume: ṁ = Q_std × MW / (V_m × 3600), V_m = R×T_std/P_std. Sizing tab shows inline equations (molar volume, mass flow, polytropic exponent, discharge temperature, head, power) as monospace cards with symbolic + numeric substitution. Stage-by-stage breakdown table. Type-aware compressor curve chart: centrifugal shows head-flow curves with surge line, BEP, min continuous flow, and operating point; reciprocating shows power vs. compression ratio with volumetric efficiency (12% clearance model) and discharge temperature. Switching type auto-updates default efficiencies and temperature limits. Stages/Summary tabs gated until calculation runs. Engine: `compressorSizing.ts` with CalcTrace.
...
- **Engineering Basis & Standards Page**: Dedicated reference page at `/engineering-basis` cataloging all industry standards and calculation methods used across the calculator suite. Organized by discipline (Relief Systems, Hydraulics, Piping Components, Separation, Heat Transfer, Rotating Equipment, Fluid Properties) with standard codes, titles, scope descriptions, and badges linking to applicable calculators. Also shows key calculation methods (Darcy-Weisbach, Souders-Brown, LMTD, Erosional Velocity, Control Valve Cv) with symbolic equations and descriptions. Linked from homepage "Engineering Basis & Standards" button.
- **Screening Tool Disclaimers**: PRD/Flare Relief (`psv-sizing.tsx`) and Thermal Expansion Relief (`thermal-relief.tsx`) calculators display a prominent amber "Screening Tool Only" banner below the page header, clearly stating the tool is not intended for final relief system design and must be validated per API 520/521/526.
