...
- **Calculator Suite**: 14 discipline-specific calculators covering hydraulics, fluid properties, equipment sizing, and relief systems.
...
- **Heat Exchanger Tube Geometry & TEMA Guidance**: The Geometry tab includes tube sizing (OD/BWG selection, tube count estimation, bundle diameter via Kern/TEMA K1/n1 constants by pitch and passes, shell ID estimate, tube-side velocity check), TEMA type selection guidance (BEM/AES/AET/BEU/AEP/AKT based on ΔT thresholds), and fluid allocation advisory (tube-side vs shell-side logic based on phase, temperature, capacity rate). Engine functions: `computeTubeGeometry()`, `generateTEMAGuidance()`, `STANDARD_TUBE_SIZES`, `TEMA_TYPES`, `BUNDLE_CONSTANTS`.
...
- **API 2000 Tank Venting Calculator**: Normal venting (thermal breathing via C×V^0.7 per API 2000 Table 2/3, liquid movement 1:1 displacement with flash factor), emergency venting (fire exposure Q=43,200×F×A_w^0.82 with environmental factors), and vent device sizing (Cd=0.62 PV valve). 5-tab wizard: Tank → Product → Normal → Emergency → Results. Engine: `api2000.ts`.
