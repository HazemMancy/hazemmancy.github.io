...
export default function LineBlanksPage() {
  ...
  const [rows, setRows] = useState<LineBlankRow[]>([]);
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">ASME B16.48 — Spectacle blinds, spades, spacers</p>
            </div>
          </div>
          <div className="mb-4"><DatasetStatus meta={meta} onImport={() => setShowImport(true)} onDelete={async () => { await deleteDataset("line-blanks"); setMeta(null); setRows([]); setSelectedIdx(null); }} /></div>
          ...
          {rows.length === 0 && !showImport && (
            <Card className="mt-6"><CardContent className="py-12 text-center"><Shield className="w-12 h-12 text-muted-foreground/20 mx-auto mb-4" /><h3 className="text-sm font-medium mb-2">No Line Blank Data Loaded</h3><p className="text-xs text-muted-foreground mb-4">Import your licensed line blank dimension data.</p><Button variant="outline" size="sm" onClick={() => setShowImport(true)}>Import Line Blank Dataset</Button></CardContent></Card>
          )}
          <div className="mt-6 p-3 rounded-md border border-muted/30 bg-muted/5"><p className="text-[10px] text-muted-foreground"><strong>Disclaimer:</strong> Dimensional tables from user-supplied licensed datasets. Reference: ASME B16.48.</p></div>
        </div>
      </section>
    </div>
  );
