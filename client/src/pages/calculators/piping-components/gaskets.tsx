...
import { GASKET_DATA, GASKET_DATA_PROVENANCE } from "@/lib/engineering/piping/data";
...
const getGasketKey = (r: GasketRow) => `${r.nps}|${r.class_rating}|${r.type}|${r.material ?? ''}`;
...
export default function GasketsPage() {
  ...
  const handleDelete = async () => {
    ...
    setMeta(null); setRows(GASKET_DATA); setSelectedKey(null); setIsCustom(false);
  };
  ...
  const selectedIdx = useMemo(() => {
    ...
    const idx = filtered.findIndex(r => getGasketKey(r) === selectedKey);
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">ASME B16.20 / B16.21 — Spiral wound, ring type, full-face gaskets — dimensional reference only</p>
            </div>
          </div>

          <div className="mb-4 flex items-center gap-3 flex-wrap">
            <Badge variant={isCustom ? "default" : "secondary"} className="text-[10px]">
              {isCustom ? `Custom: ${meta?.name ?? "Imported"}` : `Built-in: ${GASKET_DATA_PROVENANCE.edition}`}
            </Badge>
          ...
          {!isCustom && (
            <div className="mb-4 p-3 rounded-md border border-blue-900/30 bg-blue-950/10">
              <p className="text-[10px] text-muted-foreground leading-relaxed">
                <strong>Data source: {GASKET_DATA_PROVENANCE.edition}.</strong> {GASKET_DATA_PROVENANCE.disclaimer}
              </p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2"><Card>
              ...
              <CardContent className="pt-0 max-h-[500px] overflow-y-auto">
                <DimensionTable rows={filtered} columns={COLUMNS} selectedIndex={selectedIdx} onSelect={idx => setSelectedKey(getGasketKey(filtered[idx]))} />
              </CardContent>
            </Card></div>
          ...
          <div className="mt-6 p-3 rounded-md border border-amber-900/30 bg-amber-950/10">
            <p className="text-[10px] text-muted-foreground leading-relaxed">
              <strong>Dimensional Reference Tool — Not for Direct Design Use.</strong> {GASKET_DATA_PROVENANCE.disclaimer}
            </p>
          </div>
        </div>
      </section>
