...
function computeCaseResult(
  ...
  const trace: CalcTrace = {
    ...
    intermediateValues: {},
    assumptions: [
      "PRELIMINARY THERMAL SIZING / SCREENING ONLY — LMTD method with F-correction factor; not a rating tool and not final design authority",
      "Single-phase sensible heat transfer with constant Cp assumed; phase-change (condensing/boiling) is flagged and not rigorously modeled",
      "Constant properties (Cp, ρ, U) over full temperature range — no correction for property variation with temperature",
      "F-factor for 1-2 pass from Bowman et al. correlation — screening approximation only; verify with TEMA charts or vendor software",
      "No Bell-Delaware shell-side rating, no baffle cut/spacing optimization, no shell-side pressure drop",
      "No tube vibration analysis, no unsupported span check (TEMA RCB-4.6)",
      "No shell-side bypassing, maldistribution, or leakage effects modeled",
      "Tube count, bundle diameter, and shell ID are screening estimates (Coulson & Richardson / HEDH method) — not final mechanical layout",
      "Final exchanger design/rating requires HTRI, Aspen EDR, vendor thermal rating software, or equivalent detailed methods",
    ],
    warnings: [],
    flags: [],
  };
