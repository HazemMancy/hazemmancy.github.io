...
function drawTubeBundleOnPDF(
  ...
  // Note: setLineDashPattern not available in all jsPDF versions, using solid lines
  if (tubePasses >= 2) {
...
export function generateDatasheetPDF(data: DatasheetData): void {
  ...
  const addFooter = () => {
    const totalPages = (doc as any).internal?.getNumberOfPages?.() || doc.internal.pages.length - 1 || 1;
    doc.setFontSize(7);
    doc.setTextColor(100, 100, 100);
    doc.text('Generated by Heat Exchanger Sizing Calculator | TEMA/API Standards', pageWidth / 2, pageHeight - 10, { align: 'center' });
    doc.text(`Page ${currentPage} of ${totalPages} | ${data.date}`, pageWidth / 2, pageHeight - 6, { align: 'center' });
    doc.setTextColor(0, 0, 0);
  };
  ...
  const checkPageBreak = (needed: number) => {
    if (y + needed > pageHeight - footerHeight - margin) {
      doc.addPage();
      currentPage++;
      y = margin;
    }
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.setTextColor(100, 100, 100);
    doc.text('Generated by Heat Exchanger Sizing Calculator | TEMA/API Standards', pageWidth / 2, pageHeight - 10, { align: 'center' });
    doc.text(`Page ${i} of ${totalPages} | ${data.date}`, pageWidth / 2, pageHeight - 6, { align: 'center' });
  }
